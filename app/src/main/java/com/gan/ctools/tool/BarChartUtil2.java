...
public class BarChartUtil2 {
    ...
    public BarChartUtil2(Context mContext, BarChart chart){
...
//        leftAxis.setPosition(YAxis.YAxisLabelPosition.OUTSIDE_CHART);
        ...
        leftAxis.setAxisMinimum(-2f); // this replaces setStartAtZero(true)
...
//        rightAxis.setValueFormatter(custom);
        ...
        rightAxis.setSpaceBottom(1f);
        rightAxis.setAxisMaximum(2f);
        rightAxis.setAxisMinimum(-2f); // this replaces setStartAtZero(true)
...
//    public void showData(List<BarEntry> yValues, String lable, int color, ArrayList<String> xValues){
    public void showData(List<BarEntry> yValues, String lable, int maxGrap, int minGrap){
       ...
       if(Math.abs(maxGrap) - Math.abs(minGrap) >= 0){
           ...
           bar_chart.getAxisLeft().setAxisMaximum(Math.abs(maxGrap));
           bar_chart.getAxisLeft().setAxisMinimum(-(Math.abs(maxGrap)));
       }else{

           bar_chart.getAxisLeft().setAxisMaximum(Math.abs(minGrap));
           bar_chart.getAxisLeft().setAxisMinimum(-(Math.abs(minGrap)));
...
//        barDataSet.setHighLightAlpha(37);//设置点击后柱子透明度改变
        ...
        data.setBarWidth(0.8f);//设置树状图的宽度
